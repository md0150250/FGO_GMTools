...
@WebServlet("/insertBulletinServlet")
public class insertBulletinServlet extends HttpServlet {
		...
		if(ServletFileUpload.isMultipartContent(request)){//判断是不是文件上传表单，如果不是设置页面500错误
			...
			try {
				...
				for(FileItem item:items){
					if(item.isFormField()){
						//是普通元素
						if(item.getFieldName().equals("beginAt")){
							sbeginAt = item.getString();//通过对普通元素name值的判断找到该元素的值
						}else if(item.getFieldName().equals("endAt")){
							sendAt = item.getString();
						}else if(item.getFieldName().equals("rank")){
						...
						if(extName.equals(".png") || extName.equals(".jpg") || extName.equals(".gif")){
							...
							if(item.getSize()<maxSize){
								...
								item.write(file);//将图片文件上传
								 try {  
									 TestScp.putFile("/home/johnny/tomcat/webapps/img/"+fileName, "/home/johnny/img");  
							        	...
							        	request.setAttribute("Message", "上傳錯誤");
							        	request.getRequestDispatcher("index.jsp").forward(request, response);
							            e.printStackTrace();  
							            return;
							        }  
								...
								//存入database
								 int result=bs.insertBulletin(sbeginAt, sendAt, srank, "/home/johnny/tomcat/webapps/img/"+fileName);
								if(result==1){
